...
//!
//! Shared by linker resolution and `vut doctor`. On Windows `PATHEXT` is
//! honored; an explicit extension is never duplicated.
...
/// Returns the first existing executable from `names` on `PATH`.
#[must_use]
pub fn find_first_on_path(names: &[&str]) -> Option<(String, PathBuf)> {
    for name in names {
        if let Some(path) = find_on_path(name) {
            return Some(((*name).to_owned(), path));
        }
    }
    None
}
