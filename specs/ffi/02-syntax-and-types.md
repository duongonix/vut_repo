...
`bool` không nên mặc định C ABI-safe nếu representation chưa được khóa.

Nếu compiler có canonical ABI representation cho bool, spec có thể bổ sung sau.
...
```vut
extern "C" fn print_name(name: str)
```
...
hoặc future dedicated:

```text
cstr
```

nếu `cstr` được bổ sung chính thức.

MVP có thể chỉ support:
...
nếu ABI của chúng chưa được khai báo explicit.
...
FFI type checker phải reject trực tiếp:
